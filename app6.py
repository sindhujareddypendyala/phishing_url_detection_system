...
# -------------------------
st.set_page_config(page_title="Phishing URL Detection System", layout="wide", initial_sidebar_state="expanded")
...
st.markdown(
    ...
    /* page background */
    /* page background - Ash Grey Light Dark */
    /* page background - Ash Grey Light Dark */
    /* page background */
    .stApp {
    ...
    if user_url:
        ...
        if not user_url.lower().startswith(("http://", "https://")):
            ...
            if LOTTIE_AVAILABLE:
                try:
                    if pred == 1:
                        st_lottie(LOTTIE_URLS["warning"], height=160)
                    else:
                        st_lottie(LOTTIE_URLS["check"], height=160)
                except Exception:
                    pass
            ...
            if pred == 1:
                st.error(f"🚨 The model flagged this URL as PHISHING (probability {confidence_pct})", icon="🚨")
            else:
                st.success(f"✅ URL appears SAFE (phishing probability {confidence_pct})", icon="✅")
...
if page == "Bulk CSV":
    ...
    if uploaded:
        ...
        if "url" not in df_in.columns:
            st.error("CSV must contain a 'url' column (case-sensitive).")
        else:
...
if page == "About":
    ...
    st.markdown("""
    - Model: **RandomForest ** trained on a synthetic but realistic feature distribution. 
